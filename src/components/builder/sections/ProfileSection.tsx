...
import { setProfileId } from "../../../store/slices/agentSlice";
...
export const ProfileSection: React.FC<ProfileSectionProps> = ({ profiles }) => {
  const dispatch = useAppDispatch();
  const selectedId = useAppSelector((state) => state.agent.profileId);
    ...
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="grid grid-cols-1 gap-2">
        {profiles.map((profile) => (
          <button
            key={profile.id}
            onClick={() => dispatch(setProfileId(selectedId === profile.id ? "" : profile.id))}
