...
export const SkillPanel: React.FC<SkillPanelProps> = ({ availableSkills, selectedIds, onChange }) => {
  const [activeId, setActiveId] = useState<string | null>(null)
  const selectedSkills = selectedIds.map(id => availableSkills.find(s => s.id === id)).filter(Boolean) as Skill[]
  ...
  const removeSkill = (id: string) => onChange(selectedIds.filter(s => s !== id))
